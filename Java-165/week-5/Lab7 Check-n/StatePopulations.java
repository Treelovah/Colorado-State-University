    ...
    public static void main(String[] args) throws IOException {
        ...
        for (i = 0; i < zipCodeState.size(); ++i) {
            if (myZipCode == zipCodeState.get(i).getValue1()) {
                abbrevation = zipCodeState.get(i).getValue2();
                break;
            }
        }
        String state = "";
        for (i = 0; i < abbrevState.size(); ++i) {
            if (abbrevState.get(i).getValue1().equals(abbrevation)) {
                state = abbrevState.get(i).getValue2();
                break;
            }
        }

        for (i = 0; i < statePopulation.size(); ++i) {
            if (statePopulation.get(i).getValue1().equals(state)) {
