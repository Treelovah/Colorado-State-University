...
public class Recursion {
    ...
    loops). Hint: a Scanner may be helpful for finding word boundaries. After
    delimiting by space, use the following method on your String to remove
    punctuation {@code .replaceAll("[^a-zA-Z]", "")} If you use a Scanner, you
    will need a helper method to do the recursion on the Scanner object.
     *
    ...
    {
    return longestWord(words, "").length();
    }

    public static String longestWord(String words, String longestWord)
    {
        int i = Math.min(words.indexOf(' '), words.indexOf('.'));
        if (i == -1) return words.length() > longestWord.length() ? words : longestWord;
        longestWord = i > longestWord.length() ? words.substring(0, i) : longestWord;
    ...
    public static void main(String[] args) {
        ...
        isPalindrome("Recursion")); System.out.println();
        ...
        quoteOne, longestWordLength(quoteOne)); String quoteTwo = "Try to be like the turtle – at ease in your own shell.";

        System.out.printf("The longest word in the following quote:\n%s\nshould be 6 -> %d\n\n",quoteTwo, longestWordLength(quoteTwo)); System.out.println();
